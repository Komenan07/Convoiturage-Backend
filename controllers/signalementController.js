...
const creerSignalement = async (req, res, next) => {
  try {
    ...
    // Compatibilité avec le middleware qui peut utiliser id ou userId
    ...
    // Vérification que signaleId est fourni
    if (!signaleId) {
      nettoyerFichiersTemp(req.files);
      return res.status(400).json({
        success: false,
        message: 'ID de l\'utilisateur signalé requis',
        code: 'SIGNALE_ID_REQUIRED'
      });
    }

    // Vérification auto-signalement
    if (signaleId === currentUserId.toString()) {
      nettoyerFichiersTemp(req.files);
      return res.status(400).json({
        success: false,
    ...
    console.log('Vérification de l\'utilisateur signalé...');
    ...
    console.log('Utilisateur signalé trouvé:', utilisateurSignale.nom, utilisateurSignale.prenom);

    let trajet = null, message = null;
    
    // Vérification du trajet si fourni
    if (trajetId) {
      console.log('Vérification du trajet:', trajetId);
    ...
    // Vérification du message si fourni
    if (messageId) {
    ...
    if (req.files && req.files.length > 0) {
      console.log('Traitement de', req.files.length, 'fichier(s) de preuve...');
      try {
        ...
        nettoyerFichiersTemp(req.files);
        console.log('Preuves uploadées avec succès');
      } catch (error) {
        nettoyerFichiersTemp(req.files);
    ...
    console.log('Priorité calculée:', priorite);
    ...
    console.log('Signalement créé en mémoire, tentative de sauvegarde...');
    
    // Sauvegarde en base
    await nouveauSignalement.save();
    console.log('Signalement sauvegardé avec succès, ID:', nouveauSignalement._id);
    ...
    // Notifications pour les signalements prioritaires
    if (['HAUTE', 'CRITIQUE'].includes(priorite)) {
      console.log('Envoi de notifications pour signalement prioritaire...');
      try {
    ...
    if (error.errors) {
      console.error('Erreurs de validation Mongoose:', error.errors);
    }
