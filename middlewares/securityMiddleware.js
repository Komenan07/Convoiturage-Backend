  ...
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: [
        ...
        "https://api-checkout.cinetpay.com", // Ajout CinetPay
...
// Rate limiter pour les paiements - Ajusté pour CinetPay
const paymentLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 heure
  max: 15, // 15 transactions max par heure (augmenté pour retry)
  message: {
    success: false,
...
const rechargeLimiter = rateLimit({
  ...
  max: 10, // 10 recharges max par jour (augmenté)
  message: {
    success: false,
...
// =========================
// VALIDATION WEBHOOK CINETPAY SPÉCIFIQUE
// =========================
const validateCinetPayWebhook = (req, res, next) => {
  try {
    const { cpm_site_id, cmp_trans_id, cpm_result } = req.body;
    
    // Vérifier que les champs obligatoires sont présents
    if (!cpm_site_id || !cmp_trans_id || !cpm_result) {
      return res.status(400).json({
        success: false,
        error: 'CINETPAY_WEBHOOK_INVALID',
        message: 'Données webhook CinetPay invalides',
        code: 'WEBHOOK_003'
      });
    }
    ...
    if (cpm_site_id !== process.env.CINETPAY_SITE_ID) {
      console.warn('🚨 Site ID CinetPay incorrect:', {
        received: cpm_site_id,
        expected: process.env.CINETPAY_SITE_ID,
        ip: req.ip
      });
      
      return res.status(403).json({
        success: false,
        error: 'CINETPAY_SITE_ID_INVALID',
        message: 'Site ID CinetPay invalide',
        code: 'WEBHOOK_004'
      });
    }
    ...
    console.error('Erreur validation webhook CinetPay:', error);
    return res.status(500).json({
      success: false,
      error: 'WEBHOOK_VALIDATION_ERROR',
      message: 'Erreur validation webhook',
      code: 'WEBHOOK_005'
    });
  }
};
...
const securityLogger = (req, res, next) => {
  ...
  const sensitiveEndpoints = ['/auth/', '/payment/', '/paiement/', '/admin/'];
...
module.exports = {
  ...
  validateCinetPayWebhook, // Nouveau pour CinetPay
  ...
  // Middleware spécifique pour les routes de paiement CinetPay
  getCinetPaySecurityMiddlewares: () => [
    helmetConfig,
    paymentLimiter,
    authenticateToken,
    requireVerifiedUser,
  ...
  // Middleware pour webhook CinetPay
  getCinetPayWebhookMiddlewares: () => [
    validateCinetPayWebhook,
    detectMaliciousActivity,
    securityLogger
  ],
